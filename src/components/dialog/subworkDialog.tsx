...
export interface SubworkDialogValues {
  name: string;
  description?: string;
}

export interface SubworkDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  initialValues?: SubworkDialogValues;
  onSubmit: (subwork: SubworkDialogValues) => Promise<void> | void;
}

export function SubworkDialog({ open, onOpenChange, initialValues, onSubmit }: SubworkDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="fixed left-1/2 top-1/2 z-50 max-h-[90vh] w-[95vw] max-w-lg -translate-x-1/2 -translate-y-1/2 overflow-y-auto border border-slate-200/80 bg-white px-5 py-4 text-slate-900 shadow-xl shadow-slate-900/20 focus:outline-none dark:border-white/10 dark:bg-slate-950 dark:text-slate-50">
        <DialogHeader>
          <DialogTitle className="text-base">{initialValues ? "Edit Subwork" : "Add Subwork"}</DialogTitle>
          <DialogDescription className="text-xs text-slate-500 dark:text-slate-400">
            {initialValues
              ? "Update the details for this subwork."
              : "Create a new subwork under this work."}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="mt-3 space-y-4 text-xs">
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              placeholder="Short description of the subwork"
            />
          </div>
          ...
          <DialogFooter className="mt-4 flex justify-end gap-2">
            ...
            <Button type="submit" size="sm" className="text-xs" disabled={loading}>
              {loading ? "Saving..." : initialValues ? "Save changes" : "Save subwork"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
