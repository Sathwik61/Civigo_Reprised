 ...
 */
export async function getWorkLocalIdFromBackendId(workBackendId: string | undefined): Promise<number | undefined> {
...
export async function syncSubworksToServer() {
  ...
  for (const s of toCreate) {
    ...
    try {
      const Wid = await getWorkBackendIdFromLocalId(Number(s.workBackendId));
...
export async function getSubworkValue(id: number, variableName: string): Promise<any> {
